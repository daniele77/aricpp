...
public:
    ...
    Channel(Client& _client, const std::string _id, const std::string& _state = {}) : id(_id), client(&_client)
    {
        StateChanged(_state);
    }
    ...
    Proxy& Ring() const { return Proxy::Command(Method::post, "/ari/channels/" + id + "/ring", client); }

    Proxy& RingStop() const { return Proxy::Command(Method::delete_, "/ari/channels/" + id + "/ring", client); }
    ...
    Proxy& Hold() const { return Proxy::Command(Method::post, "/ari/channels/" + id + "/hold", client); }

    Proxy& Unhold() const { return Proxy::Command(Method::delete_, "/ari/channels/" + id + "/hold", client); }

    Proxy& Silence() const { return Proxy::Command(Method::post, "/ari/channels/" + id + "/silence", client); }

    Proxy& StopSilence() const { return Proxy::Command(Method::delete_, "/ari/channels/" + id + "/silence", client); }
    ...
    Proxy& StopMoh() const { return Proxy::Command(Method::delete_, "/ari/channels/" + id + "/moh", client); }

    Proxy& Answer() const { return Proxy::Command(Method::post, "/ari/channels/" + id + "/answer", client); }

    Proxy& Hangup() const { return Proxy::Command(Method::delete_, "/ari/channels/" + id, client); }

    Proxy& Continue() const { return Proxy::Command(Method::post, "/ari/channels/" + id + "/continue", client); }

#ifdef ARICPP_DEPRECATED_API
    [[deprecated("Use the method Dial instead")]] Proxy& Call(
        const std::string& endpoint, const std::string& application, const std::string& callerId,
     ...
     * @param application The stasis application that is subscribed to the originated
     channel.
    ...
    /**
     * @brief Create only an asterisk channel (once created, you can dial with
     * Channel::Dial)
     *
    ...
    Proxy& Create(const std::string& endpoint, const std::string& application) const
    {
        ...
        return Proxy::Command(
            Method::post,
            "/ari/channels/create?"s +                               //
                "endpoint=" + UrlEncode(endpoint) +                  //
                "&app=" + UrlEncode(application) +                   //
                "&channelId=" + UrlEncode(id) + "&appArgs=internal", //
            client);
    }
     ...
     * @return Proxy& You can call After() and OnError() on the returned object
     */
    Proxy& Dial() const { return Proxy::Command(Method::post, "/ari/channels/" + id + "/dial", client); }
    ...
    ProxyPar<Playback>& Play(
        const std::string& media, const std::string& lang = {}, const std::string& playbackId = {}, int offsetms = -1,
        int skipms = -1) const
    {
    ...
    Proxy& Snoop(
        const std::string& app, Direction spy = Direction::none, Direction whisper = Direction::none,
    ...
    /*! Returns the channel destroy cause as codified in Q.850
        see e.g., page
       http://support.sonus.net/display/uxapidoc/q.850+cause+codes+-+reference
...
private:
    ...
    void StasisStart(
        const std::string& _name, const std::string& _ext, const std::string& _callerNum,
        const std::string& _callerName)
    {
        name = _name;
        extension = _ext;
    ...
    void StateChanged(const std::string& s)
    {
        if (s == "Down")
            ...
            state = State::busy;
        else if (s == "Dialing Offhook")
            state = State::dialingoffhook;
        else if (s == "Pre-ring")
            state = State::prering;
        else if (s == "Mute")
            state = State::mute;
...
template<class Dummy>
const Channel::Direction Channel::DirectionBase<Dummy>::none{"none"};
template<class Dummy>
const Channel::Direction Channel::DirectionBase<Dummy>::both{"both"};
template<class Dummy>
const Channel::Direction Channel::DirectionBase<Dummy>::in{"in"};
template<class Dummy>
const Channel::Direction Channel::DirectionBase<Dummy>::out{"out"};
