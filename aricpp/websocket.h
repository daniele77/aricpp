...
public:
    ...
    void Connect(std::string req, ConnectHandler h, std::size_t _connectionRetrySeconds)
    {
        ...
        if (connectionRetrySeconds != 0) StartPingTimer();
    }
    ...
    void Close() noexcept
    {
        try
        {
            if (socket.is_open())
            {
                if (connected) websocket.close(boost::beast::websocket::close_code::normal);
...
private:
        ...
        if (connected)
        {
            ...
            websocket.async_ping(
                ...
                [this](const boost::beast::error_code& error_code)
                {
                    ...
                    {
                        // remedy the situation. Close the current websocket connection
                        // and try to restart
                        Close();
                        Resolve();
                    }
    ...
    void Read()
    {
        websocket.async_read(rxData, [this](boost::system::error_code ec, std::size_t /* bytes */) { Received(ec); });
    }
