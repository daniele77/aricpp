...
public:
    ...
    Client(
        boost::asio::io_service& ios, const std::string& host, const std::string& port, std::string _user,
        std::string _password, std::string _application)
        : user(std::move(_user)), password(std::move(_password)), application(std::move(_application)),
          websocket(ios, host, port), httpclient(ios, host, port, user, password)
    {
    }
     ...
     * @param connectionRetrySeconds the period in seconds of reconnection tries. When
     * this parameter is 0 (default) no reconnection is tried.
     */
